...
TANK_CONFIG['total_volume'] = 40000
...
def calculate_data(distance_from_sensor_cm, status='online', tank_id='tank1'):
    ...
    if distance_from_sensor_cm <= 35:
        ...
        percentage = ((empty_at_cm - distance_from_sensor_cm) /
                      (empty_at_cm - 35)) * 100
...
def receive_data():
    ...
    logger.info(
        f"{dist_raw}cm -> {current_data['volume_liters']}L "
        f"(filtrado: {nivel_filtrado:.2f}cm, "
        f"%: {current_data['level_percentage']}%, "
        f"Alerta: {alerta_ativo})"
    )
