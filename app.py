...
def calculate_data(distance_from_sensor_cm, status='online', tank_id='tank1'):
    logger.debug(f"Calculando dados para distância: {distance_from_sensor_cm} cm")
    ...
    logger.debug(f"Dados calculados: %={percentage:.2f}, Volume={liters:.2f}L")
    return {
        'tank_id': tank_id,
...
def receive_data():
    ...
    logger.info("Requisição POST recebida em /api/water-level")
    try:
        data = request.get_json()
        if not data:
            logger.error("JSON não fornecido na requisição.")
            return jsonify({"status": "error", "message": "JSON não fornecido"}), 400
            
        dist_raw = float(data.get('distance_cm', 0))
        status = data.get('status', 'online')
        logger.info(f"Dados brutos recebidos: distance_cm={dist_raw}, status={status}")
        
        # 4. Ignorar Leituras Absurdas
        if dist_raw < 10 or dist_raw > 1100:
            logger.warning(f"Leitura absurda ignorada: {dist_raw} cm (fora da faixa 10-1100cm)")
        ...
        if nivel_filtrado is None:
            ...
            logger.info(f"Filtro exponencial inicializado com {dist_raw} cm")
        else:
            nivel_filtrado = ALPHA * dist_raw + (1 - ALPHA) * nivel_filtrado
            logger.debug(f"Filtro exponencial: raw={dist_raw}, filtered={nivel_filtrado:.2f}")
        ...
        logger.debug(f"current_data atualizado: {current_data}")

        # 3. Histerese no Alerta de 20%
        if alerta_ativo:
            if current_data['level_percentage'] > 23:
                alerta_ativo = False
                logger.info("Alerta de nível baixo desativado (histerese > 23%)")
        else:
            if current_data['level_percentage'] < 20:
                alerta_ativo = True
                logger.warning("Alerta de nível baixo ativado (< 20%)")
        current_data['alert_low'] = alerta_ativo
        
        db_path = os.path.join('/tmp/', 'water_monitor.db')
        conn = sqlite3.connect(db_path)
        c = conn.cursor()
        # A instrução SQL foi corrigida para incluir todas as 8 colunas e um NULL para o ID auto-incremento
        ...
        conn.commit()
        conn.close()
        logger.info("Dados inseridos no histórico do banco de dados.")
        ...
        return jsonify({"status": "success", "tank": current_data['tank_id'], "volume": current_data['volume_liters'], "filtered_distance": round(nivel_filtrado, 2)}), 200
    except Exception as e:
        logger.error(f"Erro ao receber dados: {e}", exc_info=True) # exc_info=True para logar o traceback completo
        return jsonify({"status": "error", "message": str(e)}), 400

@app.route('/api/latest')
@app.route('/api/latest/tank1')
def get_latest():
    logger.info("Requisição GET recebida em /api/latest")
    return jsonify(current_data)

@app.route('/api/history')
def get_history():
    logger.info("Requisição GET recebida em /api/history")
    try:
        db_path = os.path.join('/tmp/', 'water_monitor.db')
        conn = sqlite3.connect(db_path)
        c = conn.cursor()
        ...
        logger.info(f"Retornando {len(history_data)} registros de histórico.")
...
@app.route('/health')
def health_check():
    logger.info("Requisição GET recebida em /health")
